...
export type BillsSort = "dueDateAsc" | "dueDateDesc" | "amountDesc" | "amountAsc" | "nameAsc" | "nameDesc";
...
export function FiltersBar(props: {
  ...
  const { search, onSearchChange, status, onStatusChange, sort, onSortChange, className } = props;
  ...
  return (
    <div className={cn("flex flex-col gap-2 md:flex-row md:items-center md:justify-between", className)}>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <Select value={status} onValueChange={(v) => onStatusChange(v as BillsStatusFilter)}>
        ...
        <Select value={sort} onValueChange={(v) => onSortChange(v as BillsSort)}>
          <SelectTrigger className="w-full rounded-2xl sm:w-[220px]">
