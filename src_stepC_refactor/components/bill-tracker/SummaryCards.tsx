...
export function SummaryCards(props: {
  ...
  const summary = React.useMemo(() => {
    ...
    const overdue = bills.filter((b) => !b.paid && startOfDay(b.dueDate) < now).length;
  ...
  return (
    <div className={cn("grid gap-4 md:grid-cols-4", className)}>
      ...
      <Card className="rounded-3xl">
        ...
        <CardContent className="text-2xl font-semibold">{summary.unpaid}</CardContent>
      </Card>
      ...
      <Card className="rounded-3xl">
        ...
        <CardContent className="text-2xl font-semibold">{summary.overdue}</CardContent>
      </Card>

      <Card className="rounded-3xl">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium text-slate-300">Unpaid total</CardTitle>
        </CardHeader>
        <CardContent className="text-2xl font-semibold">{fmtCurrency(summary.unpaidTotal)}</CardContent>
      </Card>
    </div>
  );
}
