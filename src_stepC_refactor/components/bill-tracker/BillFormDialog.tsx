...
function defaultsFromBill(bill?: Bill | null): BillFormValues {
  return {
    ...
    dueDate: bill ? toDateInputValue(bill.dueDate) : toDateInputValue(new Date()),
...
export function BillFormDialog(props: {
  ...
  const { open, onOpenChange, initialBill, onSubmit, isSubmitting, errorMessage } = props;

  const [form, setForm] = React.useState<BillFormValues>(() => defaultsFromBill(initialBill));
  ...
  const desc = initialBill ? "Update the bill details." : "Create a new bill in your account.";
        ...
        <form onSubmit={handleSubmit} className="grid gap-4">
          ...
          <div className="grid gap-2">
            ...
            <Select
              ...
              onValueChange={(v) => setForm((s) => ({ ...s, frequency: v as BillFrequency }))}
              disabled={isSubmitting}
            >
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <label className="flex items-center gap-2 text-sm text-slate-200">
              <input
                ...
                onChange={(e) => setForm((s) => ({ ...s, isSubscription: e.target.checked }))}
                disabled={isSubmitting}
            ...
            <label className="flex items-center gap-2 text-sm text-slate-200">
              <input
                type="checkbox"
                checked={form.paid}
                onChange={(e) => setForm((s) => ({ ...s, paid: e.target.checked }))}
                disabled={isSubmitting}
                className="h-4 w-4 accent-white/80"
              />
              Mark as paid
          ...
          <DialogFooter className="pt-2">
            ...
            <Button type="submit" className="rounded-2xl" disabled={!canSubmit || isSubmitting}>
